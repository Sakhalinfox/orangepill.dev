...
>**Note**: I have used [Lightning Decoder](https://lightningdecoder.com/) to decode the Lightning Address.
{: .notice--info}
...
## Setup on NGINX
Open `/etc/nginx/sites-available/<yourdomain.com>` with a text editor like nano or vim from your console that you use to SSH into your server. Replace `<yourdomain.com>` with your `domain.com`. This should also automatically update the domain file located in `/etc/nginx/sites-enabled/yourdomain.com` because of the symbolic link. 
...
1.  Edit the below file using a text editor:
	...
	Example setup with my Lightning Address:
	...
	**Details of the configuration:**
	...
	- `add_header 'Access-Control-Allow-Origin' '*';`: Adds CORS Access-Control-Allow-Origin permission which is required by some clients like [Snort.social](https://snort.social) to allow Lightning tips.


3. Reload or restart NGINX service:
...
## Setup on Apache
Redirection on Apache is achieved using the Redirect Directive. Depending on OS or your configuration you may have to edit with your text editor nano, vim or others one of these files  `/etc/apache2/sites-available/000-default.conf ` or `/etc/apache2/sites-available/example.com.conf` (In Ubuntu) or `/etc/httpd/conf.d/vhost.conf` (In CentOS) and add a redirection from `/.well-known/lnurlp/<youridentifier>` to `https://lightningaddressprovider.com/.well-known/lnurlp/<youractualwalletidentifier>`

*Source: https://www.linode.com/docs/guides/redirect-urls-with-the-apache-web-server/*

A simple implementation would be to edit the `.htaccess` in your domain's root directory and add the redirect directive as shown below:

```
Redirect /.well-known/lnurlp/<youridentifier> https://walletofsatoshi.com/.well-known/lnurlp/<youractualwalletidentifier>
<If "%{REQUEST_URI} =~ m#^/.well-known/lnurlp/#">
...
- `Redirect`: Redirection for Apache
- `/.well-known/lnurlp/<youridentifier>`: The redirect from path. All you need is to setup the `.well-known' folder.
- `https://walletofsatoshi.com/.well-known/lnurlp/<youractualwalletidentifier>`: Replace with relevant Lightning Address provider's redirect to URL.
- `<If "%{REQUEST_URI} =~ m#^/.well-known/lnurlp/#"></If>`: Use of RegEx folder path to conditionally allow CORS Access-Control-Allow-Origin only for lnurlp path for security purposes.
- `Header always set Access-Control-Allow-Origin "*"`: Adds CORS Access-Control-Allow-Origin permission which is required by some clients like [Snort.social](https://snort.social) to allow Lightning tips.
